...
**A registered [slot resolver](command-recognition.md#slot-resolvers-when-the-game-knows-what-the-speaker-left-out) is consulted before this ruling**, so where the game fills every missing required slot from its own state the winner is complete and fires -- through every gate and filter below, none of them bypassed. That offer is the one part of this check that is *not* independent of score: it is made only to a candidate that already cleared `minScore`, and resolution adds nothing to the score, because a slot nobody spoke earns no credit. So the five-element pattern above is offered the question at its `0.60`, while a shorter pattern scoring `0.25` is refused by the score gate before any resolver is reached -- the arithmetic of which patterns can and cannot reach one is in [Known Limitations](../KNOWN_LIMITATIONS.md).
