...
**The walk assumes `words` is an in-order subsequence of the tokens.** The cursor over the tokens advances only on a text match, so it resynchronises past any token no word claims — but a word matching no *remaining* token parks it, and every token from there on takes "no data". Nothing in the package can produce that: VOSK's words arrive verbatim from the JSON parser and are 1:1 with the transcript's tokens, the simulated-word helper splits the very text it is labelling, and the utterance buffer aligns each result's words to that result's own tokens. It takes a hand-built `VoxrWord[]` handed to `InjectText` or `InjectResult` that is not an in-order subsequence of the text — and the cost there is degradation per *utterance* rather than per token: the tokens from the blocking word onward go unscored, and a span left with no scored token at all reports `-1`, the gate bypassed as below rather than a wrong number.
